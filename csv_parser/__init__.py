...
# Regular expression to match a possible day range, e.g  `mon-tue` or `wed-fri`
...
class CSVParser:
    ...
    def _get_day_header_mapping(self, row_data):
        """
    ...
    def _get_day_data(self, day, row_data, day_header_mapping):
        """
        ...
        if day_header_mapping.get(day):
            ...
            value = int(row_data[day_header_mapping[day]])
    ...
    def parse(self):
        ...
        # Lets get the mapping so we know which header the day corresponds to
        day_header_mapping = self._get_day_header_mapping(row)

        # Lets try to get data for all  days. This will also make sure
        # the order of day data is correct withing the `results`
